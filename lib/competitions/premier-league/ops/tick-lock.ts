...
export async function acquireTickLock(holder = "tick"): Promise<TickLock> {
  ...
  if (opsBackend() === "mongo") {
    ...
    const existing = await col.findOne({ _id: "live-ops-tick" as never });
    ...
    const confirm = await col.findOne({ _id: "live-ops-tick" as never });
...
export async function releaseTickLock(leaseId: string | null): Promise<void> {
  ...
  if (opsBackend() === "mongo") {
    const db = await getMongoDb();
    if (!db) return;
    await db.collection("pl_ops_locks").deleteOne({ _id: "live-ops-tick" as never, leaseId } as never);
    return;
  }
  const file = fileLockPath();
