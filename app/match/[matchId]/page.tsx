...
function freshnessExplanation(freshness: MatchIntelligence["freshness"]) {
  ...
  if (freshness.status === "UPDATE_DUE") {
    return `${freshness.activeDueStage ?? "The next"} forecast refresh is currently due. The latest valid immutable forecast remains visible while the deterministic window is open.`;
  }
  if (freshness.status === "BLOCKED") {
    return `The ${freshness.activeDueStage ?? "current"} forecast stage is blocked by fixture evidence or scheduling policy. The prior immutable forecast remains visible.`;
  }
